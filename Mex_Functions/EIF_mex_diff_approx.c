...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
	...
	*/
	...
	double *I_se;
	I_se = mxMalloc(Ntot * sizeof(double));
	
	double *I_si;
	I_si = mxMalloc(Ntot * sizeof(double));
	...
	for(i = 0; i<Ntot; i++){
		...
		I_se[i] = 0; /* for each cell, one set of ODEs for each presynaptic population */
		I_si[i] = 0;
	}
	...
	for(tt = 1; tt <= NT && ns<maxns; tt++){
		...
		for(cc=0; cc<Ntot; cc++){
			...
			I_se[cc] = I_se[cc]+dt*(-I_se[cc]/tau_s[0]);
			I_si[cc] = I_si[cc]+dt*(-I_si[cc]/tau_s[1]);
			...
			if(t > (lastSpike[cc]+tauref[pc])){ /* not in refractory period */
				...
				//NEW EIF
				// v[cc] += dt*(-(v[cc]-EL[pc])+DeltaT[pc]*exp((v[cc]-VT[pc])/DeltaT[pc])+I_s[cc])/tau_m[pc];
				
				
				// v[cc] += dt*(-(v[cc]-EL[pc])+DeltaT[pc]*exp((v[cc]-VT[pc])/DeltaT[pc])
				// 	+0*I_se[cc]+0*I_si[cc]+mu_vec[pc])/(tau_m[pc])+0*sigma_vec[pc]*randn(0,1)*sqrt(dt);
				
				v[cc] += dt*(-(v[cc]-EL[pc])+DeltaT[pc]*exp((v[cc]-VT[pc])/DeltaT[pc])
					+I_se[cc]+I_si[cc]+mu_vec[pc])/tau_m[pc]+sigma_vec[pc]*randn(0,1)*sqrt(dt)/tau_m[pc];
				
				
				// v[cc] += dt*(-(v[cc]-(-54.0))+1.4*exp((v[cc]+52.5)/1.4))/20.0+sqrt(2)*15.4918*sqrt(dt)*randn(0,1)/20.0;
		...
		for(cc=0; cc<Ntot; cc++){
			...
			if((v[cc] > vth[pc]) && (ns<maxns)){
				...
				for(kk = wind[cc]; kk < (wind[cc+1]-1); kk++){
					...
					if(pc == 0){
						I_se[ipost] += wstr[kk]/(tau_s[pc]);
					}
					else{
						I_si[ipost] += wstr[kk]/(tau_s[pc]);
					}
				}
			}
